            ...
            get(ref(db, `/addedBooks`)).then(response => { 
                ...
                for (let i in result) { 
                    if (result[i].bookName === bookName) { 
                        document.querySelector(".infoAboutBook").innerHTML = `<h5 class="yearOfBookMoreInfo">${result[i].yearOfBook}</h5> <h1 class="nameOfBookMoreInfo">${result[i].bookName}</h1> <h3 class="authorOfBookMoreInfo">${result[i].authorName}</h3> <p class="descriptionOfBookMoreInfo">${result[i].bookDescription}</p> `; 
                        ...
                        for (let j in allComments) { 
                            ...
                            document.querySelector(".comments").innerHTML += `<div class="comment"> <h3>Anonim</h3> <p class="anonimComment">${allComments[j]}</p> </div>` 
                        } 
...
function addAndShowComments(key) { 
    ...
    get(ref(db, `/addedBooks/${key}/comments`)).then(response => { 
        const result = response.val(); 
        for (let i in result) { 
            document.querySelector(".comments").innerHTML += `<div class="comment"> <h3>Anonim</h3> <p>${result[i]}</p> </div>`; 
        } 
    }); 
    comment.value = ""; 
} 
